...
class Square:
    ...
    def hitting_wall(self):
        if (self.pos[0] < 2) or (self.pos[0] >= COLUMNS*2+2) or (self.pos[1] <= 4) or (self.pos[1] > ROWS*2+6):
            return True
        else:
            return False
class Snake:
    def __init__(self, surface):
        self.surface = surface
        self.is_dead = False
        self.squares_start_pos = [[COLUMNS // 2 + i, ROWS // 2] for i in range(INITIAL_SNAKE_LENGTH)]
        self.turns = {}
        self.dir = [-1, 0]
        self.score = 0
        self.moves_without_eating = 0
        self.apple = Square([randrange(2,ROWS*2+2), randrange(5,COLUMNS*2+5)], self.surface, is_apple=True)
    ...
    def is_position_free(self, position):
        if position[0] < 0 or position[0] >= COLUMNS*2+2 or position[1] <= 5 or position[1] > ROWS*2+6:
            return False
    ...
    def DFSlongest_path_to_tail(self):
            ...
            for n in neighbors:
                if distance(n, self.squares[-1].pos) > dis:
                    ...
                    if v_snake.eating_apple():
                        v_snake.add_square()
                    if v_snake.DFSget_path_to_tail():
                        path.append(n)
                        dis = distance(n, self.squares[-1].pos)
            if path:
                return [path[-1]]
    def BFSlongest_path_to_tail(self):
        neighbors = self.get_available_neighbors(self.head.pos)
        path = []
        if neighbors:
            dis = -9999
            for n in neighbors:
                if distance(n, self.squares[-1].pos) > dis:
                    ...
                    if v_snake.eating_apple():
                        v_snake.add_square()
                    if v_snake.BFSget_path_to_tail():
                        path.append(n)
                        dis = distance(n, self.squares[-1].pos)
            if path:
                return [path[-1]]

    def BFSany_safe_move(self):
        neighbors = self.get_available_neighbors(self.head.pos)
        path = []
        if neighbors:
            ...
            if v_snake.BFSget_path_to_tail():
                ...
                return self.BFSget_path_to_tail()
    def DFSany_safe_move(self):
        neighbors = self.get_available_neighbors(self.head.pos)
        path = []
        if neighbors:
            path.append(neighbors[randrange(len(neighbors))])
            ...
            if v_snake.DFSget_path_to_tail():
                ...
                return self.DFSget_path_to_tail()
    def DFSset_path(self):
        ...
        if path_1:
            ...
            v_snake.add_square()  
            path_2 = v_snake.DFSget_path_to_tail()

        if path_2:  
            return path_1  

        if self.DFSlongest_path_to_tail() and\
                self.score % 2 == 0 and\
                self.moves_without_eating < MAX_MOVES_WITHOUT_EATING / 2:

            return self.DFSlongest_path_to_tail()

        if self.DFSany_safe_move():
            return self.DFSany_safe_move()

        if self.BFSget_path_to_tail():
            return self.BFSget_path_to_tail()
        print('No available path, snake in danger!')
    def BFSset_path(self):
        ...
        if path_1:
            ...
            v_snake.add_square()  
            path_2 = v_snake.BFSget_path_to_tail()

        if path_2:  
            return path_1  

        if self.BFSlongest_path_to_tail() and\
                self.score % 2 == 0 and\
                self.moves_without_eating < MAX_MOVES_WITHOUT_EATING / 2:

            return self.BFSlongest_path_to_tail()

        if self.BFSany_safe_move():
            return self.BFSany_safe_move()
    ...
    def game_over(self,game):
        if self.is_dead == True:
            ...
            game.show_score(0, red, 'times', 1,self)
            pygame.display.flip()
    def update(self,game):
        self.draw()
        self.move()

        if self.score == ROWS * COLUMNS  - INITIAL_SNAKE_LENGTH:  
            self.won_game = True
        ...
        if self.eating_apple():
            self.add_square()
    def BFSupdate(self,game):

        self.path = self.BFSset_path()
        if self.path:
        ...
        if self.eating_apple():
            self.add_square()
    def DFSupdate(self,game):
        self.path = self.DFSset_path()
        if self.path:
            self.go_to(self.path[0])
